...
class LetterSquare(Static):
    ...
    # FIXED: The render method now delegates to the app's single text processor
    def render(self) -> str:
        """
        Renders the letter by passing it to the correct text processor from the app.
        """
        if self.letter == " ":
            return " "
            
        return self.app.text_processor.process(
