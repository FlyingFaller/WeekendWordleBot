...
def main():
    ...
    X_val[:, -n_explicit_features:] = scaler.transform(X_val[:, -n_explicit_features:])
    X_test[:, -n_explicit_features:] = scaler.transform(X_test[:, -n_explicit_features:])
    print("\nExplicit features have been scaled.")
    
    scale_pos_weight = len(negative_list) / len(positive_list)
    print(f"Calculated scale_pos_weight for class imbalance: {scale_pos_weight:.2f}")

    # --- Training XGBoost ---
    print("\nTraining XGBoost model with Early Stopping...")
    model = XGBClassifier(
        n_estimators=1000,         # Set a high number, early stopping will find the best one
        learning_rate=0.05,        # A smaller learning rate often works well with early stopping
        eval_metric='logloss',
        random_state=42,
        early_stopping_rounds=20,
        # scale_pos_weight = scale_pos_weight,
        max_depth=10,          # Limit tree depth to prevent overly complex rules
        # subsample=0.8,        # Use 80% of data for each tree
        # colsample_bytree=0.8  # Use 80% of features for each tree
    )
    
    # The model will stop training if the validation score doesn't improve for 20 rounds
    model.fit(
        X_train, y_train,
