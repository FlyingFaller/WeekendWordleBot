...
if __name__ == "__main__":
    ...
    original_answers = get_words(url=ORIGINAL_ANSWERS_URL, refetch=True, save=False) # These are the ~2400 answer words from the original wordle game (pre NYT)
    freq, idx, word = get_minimum_freq(original_answers)
    all_5letter_words = get_words("data/en_US-large.txt")
    reduced_answers = filter_words_by_suffix(answers, all_5letter_words, suffixes=['s','es', 'ed', 'd']) # This gets rid of suffixes that should not occur (plurals/past tense)
    reduced_answers = filter_words_by_occurance(reduced_answers, min_freq=2*freq) # This gets rid of words that are not common enough. I play with the min_freq value.  
